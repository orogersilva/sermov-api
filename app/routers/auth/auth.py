...
from app.routers.auth.models import User, UserInDb, Token, TokenData, Users
...
async def get_current_user(db: db_dependency, credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer())) -> UserInDb:
    ...
    if user is None:
        raise credentials_exception
    return UserInDb(username=user.username, password=None, email=user.email, hashed_password=user.hashed_password)
